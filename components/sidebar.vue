<template>
  <div class="sidebar">
    ...
    <h1 class="heading-2" style="margin-top:15px;">Personalisasikan pengalaman Anda</h1>
    <div class="div-block-3">
      ...
      <button class="sidebar-link" :class="{'is-active': $store.state.activePage === 'StepTagline', 'is-completed': $store.state.tagline.length > 10}" id="StepTagline" @click="changeStep($event)">Menandai<span v-if="$store.state.tagline.length > 10"> 👍</span></button>
      ...
      <button class="sidebar-link" :class="{'is-active': $store.state.activePage === 'StepWhatWeDo', 'is-completed': $store.state.whatWeDo.length > 10}" id="StepWhatWeDo" @click="changeStep($event)">Apa yang akan kita lakukan<span v-if="$store.state.whatWeDo.length > 10"> 👍</span></button>
      <button class="sidebar-link" :class="{'is-active': $store.state.activePage === 'StepWhatIProvide', 'is-completed': $store.state.whatIProvide.length > 10}" id="StepWhatIProvide" @click="changeStep($event)">Apa yang akan saya berikan<span v-if="$store.state.whatIProvide.length > 10"> 👍</span></button>
      ...
      <button class="sidebar-link" :class="{'is-active': $store.state.activePage === 'StepWhereWeBe', 'is-completed': $store.state.whereWeBe.length > 10}" id="StepWhereWeBe" @click="changeStep($event)">Di mana kita akan berada<span v-if="$store.state.whereWeBe.length > 10"> 👍</span></button>
      <button class="sidebar-link" :class="{'is-active': $store.state.activePage === 'StepWhereWeMeet', 'is-completed': validateWhereWeMeet}" id="StepWhereWeMeet" @click="changeStep($event)">Dimana kita akan bertemu<span v-if="validateWhereWeMeet"> 👍</span></button>
      ...
      <button v-if="$store.state.status && !$store.state.status.isApproved" class="sidebar-link" :class="{'is-active': $store.state.activePage === 'StepReviewSubmit'}" id="StepReviewSubmit" @click="changeStep($event)">Tinjau dan Kirim</button>
    </div>
  </div>
</template>
