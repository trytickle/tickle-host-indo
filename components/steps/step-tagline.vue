<template>
  <div class="main-content" style="padding-bottom:100px;">
    ...
    <div>
      <form>
        <label style="margin-bottom:10px;">{{$t("whatIsTagline")}}</label>
        <input type="text" maxlength="80" name="tagline" :placeholder="$t('yourTagline')" class="text-field" autocomplete="off" v-model="$store.state.tagline">
        <p><strong :class="{'bold-text': $store.state.tagline.length >= 80}" v-text="characterCountLeft + ' '+$t('charRemaining')"></strong></p>
        <input type="submit" :value="$t('save')" class="submit-button" @click.prevent="nextClicked" :disabled="$store.state.tagline.length <= 10">
      </form>
    </div>
  </div>
</template>
