<template>
  <div class="main-content" style="padding-bottom:100px;">
    <h3 class="heading-3">{{$t('addAvailability')}}</h3>
    <div class="availability-content">
      <Calendar></Calendar>
      <div style="padding-left:30px;">
        <div><h4>{{$t('selectDates')}}</h4></div>
        <div v-if="this.$store.state.selectedDates.length <= 0" style="color:#888;margin-top:15px;">{{$t('selectDateNote')}}</div>
        ...
        <div v-if="this.$store.state.selectedDates.length > 0" style="margin-top:30px;">
          <hr>
          <h4>{{$t('setAvailabilityDetails')}}</h4>
...
export default {
  data() {
    return {
      ...
      errorMessage: "",
      showError: false,
      addAvailabilityButtonText: this.$t("addAvailability"),
      guests: [
        "1",
        "2",
        "3",
  ...
  methods: {
    ...
    dateClicked(date) {
      ...
      this.$store.commit("removeSelectedDate", date);
    },
    convertDateToString(date) {
      return moment(date)
        .locale("id")
    ...
    async addAvailabilities() {
      ...
      if (
        this.times.indexOf(this.startTime) >= this.times.indexOf(this.endTime)
      ) {
        this.errorMessage = "Waktu akhir harus lebih awal dari waktu mulai.";
        this.showError = true;
        return;
      }
      if (!this.price || this.price < 50000) {
        this.errorMessage = "Harga harus IDR 50000 atau lebih.";
        this.showError = true;
        return;
      }
      const body = {
      ...
      try {
        this.addAvailabilityButtonText = this.$t("adding") + "...";
        ...
        this.addAvailabilityButtonText = this.$t("addAvailability");
