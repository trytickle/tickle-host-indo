    ...
    <div class="modal-overlay" :hidden="!showVerifyModel">
      <div v-if="$store.state.user" class="modal-delete-experience">
        ...
        <div v-if="!$store.state.user.isVerified">
          <h4>Add IC to verify your account.</h4>
          You need to upload front and back side photo of your IC.
        </div>
         <div v-if="$store.state.user.isVerified">
          <h4 style="color: green;">You have already verified.</h4>
        </div>
        <div v-if="!$store.state.user.isVerified"><button class="delete-button" style="outline:none;margin-top:30px;margin-bottom:10px;" @click.prevent="pickPhoto(0)">{{frontVerifyFile ? "Added" : "Upload Front"}}</button>
        <button class="delete-button" style="outline:none;margin-top:30px;margin-bottom:10px;margin-left:10px" @click.prevent="pickPhoto(1)">{{backVerifyFile ? "Added" : "Upload Back"}}</button>
        </div>
        <p v-if="showVerifyError" style="color: red; margin:5px">{{verifyErrorMessage}}</p>
        <button v-if="!$store.state.user.isVerified" class="delete-button" style="outline:none;margin-top:30px;margin-bottom:10px;" @click.prevent="saveAndVerify()">{{verifyButtonText}}</button>
      </div>
    </div>
  </div>
</template>
